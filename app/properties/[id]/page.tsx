...
import { MapPin, Bed, Bath, Square, Heart, Share2, Phone, Mail, ArrowLeft, CheckCircle, Star } from 'lucide-react'
...
export default function PropertyDetailsPage() {
  ...
  const fetchProperty = async () => {
    try {
      ...
      const data = await api.getPropertyById(params.id as string)
      setProperty(data)
      setLoading(false)
    } catch (error) {
  ...
  const checkIfSaved = async () => {
    if (!isSupabaseConfigured() || !property) {
      setIsSaved(false)
      return
    }
    ...
    try {
      ...
      const saved = await supabaseApi.isPropertySaved(user.id, property.id)
      setIsSaved(saved)
    } catch (error) {
      setIsSaved(false)
    }
  ...
  const handleSave = async () => {
    if (!isSupabaseConfigured()) {
      toast('Please login to save properties', { icon: 'ℹ️' })
      return
    }
    ...
    try {
      ...
      const newSavedState = await supabaseApi.toggleSaveProperty(user.id, property.id)
      setIsSaved(newSavedState)
      toast.success(newSavedState ? 'Saved property' : 'Removed from saved')
    } catch (error: any) {
