...
class _ScheduleVisitScreenState extends State<ScheduleVisitScreen> {
  ...
  Future<void> _scheduleVisit() async {
    ...
    if (user == null) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text('Please login to schedule a visit', style: GoogleFonts.poppins()),
    ...
    if (success) {
      // Send notification message to property owner via chat
      await _notifyPropertyOwner(chatProvider, user);

      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text(
            'Visit scheduled successfully! Owner has been notified.',
  ...
  Future<void> _notifyPropertyOwner(ChatProvider chatProvider, dynamic user) async {
    // Check if chat already exists with property owner
...
📧 Email: ${_emailController.text}$notesText

Please confirm the visit or suggest an alternative time.
''';
