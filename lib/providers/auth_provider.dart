...
class AuthProvider with ChangeNotifier {
  ...
  Future<bool> login(String email, String password) async {
    ...
    try {
      ...
      if (response['success'] == true && response['data'] != null) {
        ...
        // Save tokens - handle both possible response formats
  ...
  Future<bool> register(String name, String email, String phone, String password, UserType userType) async {
    ...
    try {
      ...
      if (response['success'] == true && response['data'] != null) {
        ...
        _currentUser = User(
          id: userData?['id'] ?? '',
          name: profile?['name'] ?? userData?['user_metadata']?['name'] ?? name,
          email: profile?['email'] ?? userData?['email'] ?? email,
          phone: profile?['phone'] ?? userData?['user_metadata']?['phone'] ?? phone,
          profileImage: profile?['avatar_url'],
          userType: userType,
          favoriteProperties: List<String>.from(profile?['favorite_properties'] ?? []),
          createdAt: DateTime.tryParse(profile?['created_at'] ?? '') ?? DateTime.now(),
  ...
  Future<void> checkLoginStatus() async {
    ...
    if (_isLoggedIn) {
      ...
      // Notify immediately so UI shows logged in state
      notifyListeners();

      // Try to get updated user from API in background
      try {
        var response = await ApiClient.getCurrentUser().timeout(
